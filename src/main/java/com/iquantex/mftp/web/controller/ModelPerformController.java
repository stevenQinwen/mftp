...
public class ModelPerformController extends BaseController{
	...
	public @ResponseBody ResultObj getModel_HistoryPerform(@PathVariable String begindate,@PathVariable String enddate) {
		...
		ResultObj resultObj=null;
		 String sql = "\r\n" + 
		 		"SELECT t.generate_time AS 'generate_time',t.current_day_actual_buy_amounts,t.current_day_actual_redeem_amounts,t.current_day_actual_redeem_amounts,t.pd_next_1_day_buy_amounts \r\n" + 
		 		"FROM \r\n" + 
		 		"(SELECT * FROM model_perform WHERE generate_time>=\'"+begindate+"\' AND generate_time<=\'"+enddate+"\' ORDER BY generate_time DESC) t \r\n" + 
		 		"GROUP BY DATE_FORMAT(t.generate_time, \"%Y%m%d\") ";
...
//						String featureName_CN = ColumnsNameMapingUtils.transform2ChineseName(featureName);
			 ...
			 errorReturn(ErrorCode.OK, "数据不存在，请检查！");
		 }
		    return resultObj;
	}
	 ...
	 * 
	 */
	@RequestMapping(method = RequestMethod.GET, value = "/today_model_predict")//待定
	public @ResponseBody ResultObj getTodayModel_perform() {
		ResultObj resultObj=null;
	    ...
	    String sql3="SELECT generate_time AS \"date\" ,1 AS delta,t.pd_next_1_day_buy_amounts,t.pd_next_1_day_redeem_amounts  FROM  (SELECT * FROM model_perform  ORDER BY generate_time DESC LIMIT 1 ) t\r\n" + 
	    		"UNION \r\n" + 
	    		"SELECT generate_time AS \"date\" ,2 AS delta, t.pd_next_2_day_buy_amounts ,t.pd_next_2_day_redeem_amounts  FROM  (SELECT * FROM model_perform  ORDER BY generate_time DESC LIMIT 1 ) t\r\n" + 
	    		"UNION \r\n" + 
	    		"SELECT generate_time AS \"date\" ,3 AS delta, t.pd_next_3_day_buy_amounts,t.pd_next_3_day_redeem_amounts  FROM  (SELECT * FROM model_perform  ORDER BY generate_time DESC LIMIT 1 ) t\r\n" + 
	    		"UNION \r\n" + 
	    		"SELECT generate_time AS \"date\" ,4 AS delta, t.pd_next_4_day_buy_amounts,t.pd_next_4_day_redeem_amounts  FROM  (SELECT * FROM model_perform  ORDER BY generate_time DESC LIMIT 1 ) t\r\n" + 
	    		"UNION \r\n" + 
	    		"SELECT generate_time AS \"date\" ,5 AS delta, t.pd_next_5_day_buy_amounts,t.pd_next_5_day_redeem_amounts  FROM  (SELECT * FROM model_perform  ORDER BY generate_time DESC LIMIT 1 ) t\r\n" + 
	    		"UNION \r\n" + 
	    		"SELECT generate_time AS \"date\" ,6 AS delta, t.pd_next_6_day_buy_amounts,t.pd_next_6_day_redeem_amounts  FROM  (SELECT * FROM model_perform  ORDER BY generate_time DESC LIMIT 1 ) t\r\n" + 
	    		"UNION \r\n" + 
	    		"SELECT generate_time AS \"date\" ,7 AS delta, t.pd_next_7_day_buy_amounts,t.pd_next_7_day_redeem_amounts  FROM  (SELECT * FROM model_perform  ORDER BY generate_time DESC LIMIT 1 ) t";
...
//		Map<String,Double> map = modelPerformDao.selectModel_performByDate(sqlBean);
		 ...
		 if(list!=null&&list.size()!=0) { //如果是成功就返回正常的数据
